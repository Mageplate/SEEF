...
class MethodEvaluator:
    ...
    def _getCOSMICData(self):
        ...
        if not os.path.exists(
            self.dataPath + "/COSMIC_v3.3.1_SBS_GRCh37.txt"
        ) or not os.path.exists(self.dataPath + "/COSMIC_v3.3.1_SBS_GRCh38.txt"):
            print("Downloading COSMIC data...")
            urlretrieve(
                "https://cancer.sanger.ac.uk/signatures/documents/2046/COSMIC_v3.3.1_SBS_GRCh37.txt",
                self.dataPath + "/COSMIC_v3.3.1_SBS_GRCh37.txt",
            )
            urlretrieve(
                "https://cancer.sanger.ac.uk/signatures/documents/2047/COSMIC_v3.3.1_SBS_GRCh38.txt",
                self.dataPath + "/COSMIC_v3.3.1_SBS_GRCh38.txt",
            )
            print("Done")

        GRCh37 = pd.read_table(
            self.dataPath + "/COSMIC_v3.3.1_SBS_GRCh37.txt", index_col=0
        )
        GRCh38 = pd.read_table(
            self.dataPath + "/COSMIC_v3.3.1_SBS_GRCh38.txt", index_col=0
        )
